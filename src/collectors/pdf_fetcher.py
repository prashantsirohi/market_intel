...
from collectors.http_utils import DEFAULT_HEADERS
...
class PdfFetcher:
    def __init__(
        ...
        if session is not None:
            ...
            headers["Referer"] = "https://www.nseindia.com/corporates-listing/corporate-filings-announcements"
    ...
    def fetch(self, url: str) -> PdfFetchResult:
        ...
        # Warm up session for NSE PDFs (skip for external sessions that already have cookies)
        if not self._external_session and ("nsearchives" in url or "nseindia" in url):
            try:
        ...
        # For BSE PDFs, ensure proper Referer header
        if "bseindia" in url:
            self.session.headers["Referer"] = "https://www.bseindia.com/corporates/ann.html"
