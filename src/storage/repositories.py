...
class FilingDocument:
    ...
    llm_insight_json: Optional[str] = None
...
class FilingDocumentRepository:
    def __init__(self, db: Database):
        self.db = db
    ...
    def upsert(
        ...
        *,
        ...
        with self.db.get_connection() as conn:
            existing = conn.execute(
                "SELECT document_id FROM filing_document WHERE source_url = ?",
                [source_url],
            ).fetchone()
            ...
            if existing:
                conn.execute(
                    ...
                    UPDATE filing_document SET
                        ...
                        downloaded_at = CASE WHEN ? IS NOT NULL AND local_path IS NOT NULL THEN CURRENT_TIMESTAMP ELSE downloaded_at END,
                        extracted_at = CASE WHEN ? IS NOT NULL THEN CURRENT_TIMESTAMP ELSE extracted_at END
                    WHERE source_url = ?
                    """,
                    [
                        local_path, content_hash, mime_type, file_size,
                        pdf_status, extraction_method, extracted_text,
                        download_attempts, error_message,
                        local_path, extracted_text, source_url,
                    ],
                )
                doc_id = existing[0]
            else:
                ...
                conn.execute(
                    ...
                    INSERT INTO filing_document(
                        raw_event_id, source_url, local_path, content_hash,
                        mime_type, file_size, pdf_status, extraction_method,
                        extracted_text, download_attempts, error_message,
                    ...
                    [
                        ...
                        now if extracted_text else None,
                    ],
                )
                row = conn.execute(
                    "SELECT document_id FROM filing_document WHERE source_url = ?",
            ...
            return self.get_by_id(doc_id) if doc_id else FilingDocument(
                raw_event_id=raw_event_id, source_url=source_url,
            )

    def get_by_id(self, document_id: int) -> Optional[FilingDocument]:
        with self.db.get_connection(read_only=True) as conn:
            row = conn.execute(
                "SELECT * FROM filing_document WHERE document_id = ?",
                [document_id],
            ).fetchone()
            return self._row_to_doc(row) if row else None

    def get_by_raw_event(self, raw_event_id: int) -> Optional[FilingDocument]:
        with self.db.get_connection(read_only=True) as conn:
            row = conn.execute(
                "SELECT * FROM filing_document WHERE raw_event_id = ?",
                [raw_event_id],
            ).fetchone()
            return self._row_to_doc(row) if row else None

    def get_by_url(self, source_url: str) -> Optional[FilingDocument]:
        with self.db.get_connection(read_only=True) as conn:
            row = conn.execute(
                "SELECT * FROM filing_document WHERE source_url = ?",
                [source_url],
            ).fetchone()
            return self._row_to_doc(row) if row else None

    def list_pending_download(self, limit: int = 50) -> list[FilingDocument]:
        with self.db.get_connection(read_only=True) as conn:
            rows = conn.execute(
                """
                SELECT * FROM filing_document
                WHERE pdf_status IN ('pending', 'needs_download')
    ...
    def list_pending_extraction(self, limit: int = 50) -> list[FilingDocument]:
        with self.db.get_connection(read_only=True) as conn:
            rows = conn.execute(
                """
                SELECT * FROM filing_document
                WHERE pdf_status = 'downloaded' AND extracted_text IS NULL
                ORDER BY document_id
                LIMIT ?
                """,
                [limit],
    ...
    def _row_to_doc(self, row: Any) -> Optional[FilingDocument]:
        if row is None:
            return None
        return FilingDocument(
            ...
            llm_insight_json=row[10],
            download_attempts=row[11] or 0,
            downloaded_at=_dt(row[12]),
            extracted_at=_dt(row[13]),
